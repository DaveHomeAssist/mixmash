...
- Strict paid-test and full-golden reports are deterministic committed evidence. They SHA-256 every scoped PNG and editable source; `machineReady` is true only when byte coverage, strict validation, and the content-hashed runtime index all pass.
- The runtime resolves requested state through the DEC-79 fallback ladder, fetches each indexed PNG once, decodes through `createImageBitmap`, and reuses the cached bitmap.
...
A clean export or attractive standalone image is not approval. The review surface remains locked unless its strict report is machine-ready; after every human check passes at 1.0 zoom, it records local evidence and exports a JSON receipt bound to the exact package hash for the canonical repo record.
